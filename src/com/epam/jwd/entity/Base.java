...
public class Base {
	...
	private final int randomTime = (int) (Math.random() * (MAXIMUM_LOAD_TIME - MINIMUM_LOAD_TIME + 1)
			+ MINIMUM_LOAD_TIME);
	...
	public void acceptVanBehindTheBase(Van van) throws InterruptedException {
		...
		logger.log(Level.INFO,
				van.getName() + " arrived in line behind the base. Perishable products: " + van.isPerishableProducts());
	...
	private void getTerminal(Van van) throws InterruptedException {
		try {
			...
			logger.log(Level.INFO,
					van.getName() + " went to the base. Perishable products: " + van.isPerishableProducts() + "\n"
		...
		logger.log(Level.INFO,
				van.getName() + " waiting for a terminal. Perishable products: " + van.isPerishableProducts());
		if (!van.isPerishableProducts()) {
			try {
		...
		try {
			...
			logger.log(Level.INFO,
					van.getName() + " terminal received. Perishable products: " + van.isPerishableProducts());
		} finally {
	...
	public void workInTerminal(Van van) throws InterruptedException {
		logger.log(Level.INFO,
				van.getName() + " went to the terminal. Perishable products: " + van.isPerishableProducts());
		if (van.isPerishableProducts()) {
			countPerishableProductsInBase.set(countPerishableProductsInBase.get() - 1);
		}
		...
		if (van.getProcess() == Van.Process.LOAD) {
			logger.log(Level.INFO,
					van.getName() + " started downloading. Perishable products: " + van.isPerishableProducts());
		} else {
			logger.log(Level.INFO,
					van.getName() + " started unloading. Perishable products: " + van.isPerishableProducts());
		}
